...
class Rewind @JvmOverloads constructor(
    context: Context,
    ...
    init {
        ...
        try {
            ...
            circleColor =
                typedArray.getColor(
                    R.styleable.Rewind_r_circle_color,
                    Color.parseColor(CIRCLE_COLOR)
                )
            itemStrokeWidth =
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        arcBorderRect.set(
            centerX + arcMargin,
            arcMargin,
            width.toFloat() - arrowMargin / 2,
            height.toFloat() - arrowMargin / 2
        )
        arcCenterPointF.x = (arcBorderRect.right - arcBorderRect.left) / 2 + arcBorderRect.left
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        arcBorderRect.left = centerX + arcMargin + radiusScaleSize
        arcBorderRect.top = arcMargin + radiusScaleSize
        arcBorderRect.right = width - arrowMargin / 2 - radiusScaleSize
        ...
        canvas?.drawCircle(
            arcCenterPointF.x,
            arcCenterPointF.y,
            (arcBorderRect.right - arcBorderRect.left) / 2,
            paintCircle
        )
    ...
    fun setSeekBackwardsInSeconds(seconds: Int) {
        textInput = seconds
        invalidate()
    }
    ...
    private fun rotateAnimation(
        ...
        rotateAnimator = ValueAnimator.ofFloat().apply {
            ...
            addUpdateListener {
                angle = (it.animatedValue as Float) * -1
                invalidate()
            }
            duration = animationDuration
            interpolator = timeInterpolator
    ...
    companion object {
        ...
        private const val END_SCALE = 0.04f
